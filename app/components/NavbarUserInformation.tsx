...
export default function NavbarProdeUsers() {
  ...
  function userDisplayName() {
    //me devuelve las iniciales o un icono de anonimo
    if (firebaseUser) {
      if (firebaseUser.isAnonymous) {
      ...
      if (firebaseUser.email) {
        return (
          firebaseUser.email.split("@")[0][0] +
          firebaseUser.email.split("@")[0][1]
        );
      }
    }
    ...
    return (
      <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
        <div className="py-0">
          <Menu.Item>
            {({ active }) => (
              <div className="bg-juan text-white block px-4 py-1 text-xl first-letter:text-2xl font-bold text-right rounded-md rounded-b-none">
          ...
          <Menu.Item>
            {({ active }) => (
              ...
              >
                {AnonUsername ? "Tu usuario es " : "NO estas logueado"}
                {AnonUsername ? <b>{AnonUsername}</b> : ""}
              </a>
            )}
          </Menu.Item>
          <Menu.Item>
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <div className="relative z-20 ">{userPopup}</div>
      </Transition>
    </Menu>
  );
}

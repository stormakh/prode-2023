...
const AnonUsernameInputSchema = z.string().refine(
	(value) => {
		return (
			!value.includes(".") &&
			!value.includes("?") &&
		...
		<>
			<form className="flex" onSubmit={handleSubmit}>
				<input
					...
					className="w-5/6 border border-teal-500 text-teal-500  rounded-md rounded-r-none p-2 focus:outline-none   placeholder:text-teal-500 placeholder:font-bold placeholder:text-center"
				></input>
				<button className="bg-teal-500 flex-grow rounded-md rounded-l-none flex justify-center items-center">
					{
						<AiOutlineArrowRight
							size="24"
							style={{ color: "white" }}
			...
			{AnonUsernameError && (
				<p className="text-red-500 text-xs">{AnonUsernameError}</p>
			)}
		</>
	);
}
